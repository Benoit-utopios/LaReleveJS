...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const activityCounts: { [key: string]: number } = { "Travail": 0, "Sport": 0, "Repos": 0 };
  ...
  // ---------- INITIALISATION ----------
  const sleepChart = document.getElementById('sleepChart') as HTMLCanvasElement;
  const stepsChart = document.getElementById('stepsChart') as HTMLCanvasElement;
  const activitiesChart = document.getElementById('activitiesChart') as HTMLCanvasElement;
